...
## Fixes
- **Time tracker could get permanently stuck at 0h:00m:00s while genuinely clocked in**, on any machine whose local system clock is behind the real time. The on-screen ticker used to recompute elapsed as "now minus server check-in time" on every tick — a behind clock makes that subtraction negative forever, which was silently clamped to 0 forever, even though the check-in time and "Clocked in" status both displayed correctly (they don't depend on this math). Fixed by anchoring the ticker to a single local-clock snapshot taken at clock-in/resync, so every following tick only ever measures local-time-since-anchor and can no longer be pinned by a skewed clock. A visible warning now also appears if a real clock skew is detected, instead of silently showing a plausible-but-wrong 0.
